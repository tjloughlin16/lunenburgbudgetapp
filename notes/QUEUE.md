...
|---|---|---|---|
| A0 | **TENURE AND TURNOVER, BY DEPARTMENT — the top of this list.** TJ, 22 September 2026: *"OH shoot. TENURE. We need a tenure report on the town-personnel page. I think we can calculate this now. Whats the statistics about tenure for each department. That is, maybe average, median, max? Or some distribution or something. This goes well with turnover rates."* | **It is calculable now and it was not a week ago.** `org-chart.csv` is 6,931 rows joining a NAMED PERSON to a department, a role and a fiscal year, across fifteen books — so a person's tenure is the span of years their name appears under a unit, and turnover is the count of names that stop appearing. Per department: median, mean, max, and the distribution; plus a departure count per year. Nothing else in this project measures how long anybody stays. | **Four things that must be settled before a figure is published, and each one can make the number wrong:** (1) a roster is a POINT IN TIME, undated within its year, so a span of N appearances is at most N years and at least N−1 — state which. (2) a gap year is not a departure: the Assessing office published no roster in several years, and a department that skips a book would show its whole staff leaving and returning. Tenure must be measured only across years the unit actually PUBLISHED, and the gap years named. (3) names are OCR and vary — `Games P. Marino` for `James P. Marino`, `Jerry Beloin` and `Gerry Beloin`, `Michael-Ray Jeffreys` and `Michael Ray-Jeffreys`. A fuzzy identity is a decision, not a reading, and two spellings of one person read as two people each with half the tenure. (4) FY2011 is the left edge of the archive: anybody present in the first year has tenure that is CENSORED, not short. That is survival analysis, and reporting a median without it understates every long-serving department. **And the epistemic line (rule 7):** a name leaving the roster is a name leaving the roster. It is not a resignation, a retirement, a cut post or a person — the same distinction A10 turns on. Turnover here is a rate of NAMES not reappearing, and the page has to say so. | open |
