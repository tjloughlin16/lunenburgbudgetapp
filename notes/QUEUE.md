...
| A11 | **Finish attributing the report signatures to departments** | `extract_report_signatures.py` reads 184 signature blocks across 14 years — the named head of a body, with a title and a page — and is the only source for departments that publish no roster at all. 104 are attributed via the town's contents page and 80 are not. **The 80 are attributable**: opening their pages shows the department plainly (`far outgrown its police department`, the senior-centre report, `Director of Special Services`), and the heading tracker holds a value that is not reaching the output. Four attempts so far, each reported as progress before the end-to-end result was checked — so the next one gets verified against the 80 before anything is said about it. | open |
