...
**Four routes in, and the school budget document shows one of them.** Tax, Chapter 70 aid,
free cash and local receipts mix in the general fund, are voted at Town Meeting and become
the school appropriation — $26,323,868 for departments 300 and 301 in FY2026. Federal and
state grants, fees paid by families, and gifts land in their own funds and are **spent
straight out of them**: no vote, no appropriation, no budget line. $1,736,376 across 21 of
the 61 school funds in FY2026.

So the document residents read, argue about and vote on describes about **94%** of what the
schools spend and says nothing about the rest. That is not concealment — it is how
municipal accounting works — but it is why a budget line is net, and why Chapter 70 cannot
be followed to anything it paid for.

Drawn, and the page is the long form of this section:

    open notes/reference/data-model/money-in.html

An inventory rather than a sample — all 80 general fund revenue accounts and all 21 active
Funds are accounted for, and the columns tie: $26,359,645 + $514,410 + $1,184,493 +
$37,473 = $28,096,021. It also carries the six-level trace of a single dollar, the level
each trail goes cold at, every Fund with what feeds it, and the same question asked of the
other 64 town departments.

Also published, for reading on a phone:
https://claude.ai/code/artifact/c2d811f8-8a3b-4c81-a573-2f945cb7dae6

**The figures on that page are typed into the HTML.** Every other hand-built page in
`data-model/` has gone stale exactly that way — `match-matrix.html` and
`after-request.html` were both claiming the archive lacked years it holds within a day of
being written, which is why their grids are generated now by
`scripts/build_data_model_grids.py`. This one has the same defect and no generator yet.
Rule 2 applies to it and it is the first thing to fix.

Every box on that diagram decomposes — the appropriation into 350 lines and 45 function
codes, the grant funds into individual awards, the revenue side into 222 accounts. Those
are separate pictures and none of them is drawn yet.
