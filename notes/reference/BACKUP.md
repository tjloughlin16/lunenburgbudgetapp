...
| `sources/data/` | tree; **2 of 199 nowhere else** | 61 MB | 199 | The datasets themselves, plus provenance and the extraction plan. CSV only — the directory total below includes the two above. | seconds, given everything above |
| `sources/data/lunenburg.db` | tree + bucket | 40 MB | 1 | Derived read model. Dropped and rebuilt from the CSVs on every run, never edited. | seconds — `python3 scripts/build_db.py` |
