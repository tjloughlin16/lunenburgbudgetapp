...
disk — that is where they live.**
    ...
    open notes/reference/data-model/*.html
...
| `money-in.html` | every source paying into the school budget, grouped, and the level each trail goes cold at |
...
Nothing here is a source. Every figure on every page is computed from
`sources/data/lunenburg.db` or from `fy28/public/data/ledger.json`; re-run the scripts
before quoting anything.

**Most of these ARE now published**, byte for byte, at `/reference/<filename>` — by
`scripts/build_reference_pages.py`, which also says which of them are deliberately NOT
published and why. Four of the HTML pages here are not: `town-report.html` and `after-request.html` are about a
records request rather than about the town's money, `money-flow.html` is the superseded
twin of `school-money-flow.html`, and `follow-the-money.html` describes a defect in our own
build that was fixed on 3 September 2026 and that nothing regenerates the page to notice.
Read that script's docstring before adding a file here, because a new page that is in
neither its published list nor its excluded list fails the build.
