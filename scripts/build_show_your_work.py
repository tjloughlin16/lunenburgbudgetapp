...
def s_projection():
    ...
    w('### What goes in, on the cost side',
      ...
      f'Costs take the whole {usd(F["stm_addbacks"])} of restored salary. Revenue takes',
...
ASSUMPTIONS = [
    ...
    ('state_aid_growth', 'State aid growth', 0.01, 'derived',
     'The median annual step in the town’s own Subtotal State Aid line across 23 '
     'consecutive budget years, FY2005–FY2027 — see below'),
...
def s_register():
    ...
    # THIS SECTION USED TO NAME TWO RATES AND SAY NEITHER HAD BEEN CHANGED. One has.
    # Rule 14: the prose that justified a choice outlives the number it rested on, and
    # a commit that moves four figures and zero sentences leaves the site publishing an
    # argument for something it no longer does. So the heading, the count and the closing
    # paragraph are all derived from the grades below rather than written.
    _bare = [g for g in ASSUMPTIONS if g[3] == 'BARE']
    _n = {1: 'one', 2: 'two', 3: 'three'}.get(len(_bare), str(len(_bare)))
    w(f'### The {_n} with nothing behind {"it" if len(_bare) == 1 else "them"}',
      '',
      f'Local receipts are assumed to grow at {pct(A["local_receipts_growth"], 1)} a year.',
      ...
      f'{pct(A["state_aid_growth"], 2)} — the MEDIAN annual step in the town’s own',
      ...
      f'— about {usd(taxbase.CH70["aid"])} of a {usd(F["lps_appropriation"])} school',
    ...
    w('### Assumptions that do not affect the projection',
      ...
      table(['assumption', 'currently', 'affects', 'standing'],
            [['Sibling mix in athletics',
             ...
             ['Health enrollment by plan and tier',
      ...
      table(['section', 'built by', 'from'],
            [['1, 2, 3 — the projection', '`model/finance.py`',
             ...
             ['6 — health insurance', '`model/health.py`',
              'the Town’s open enrollment notice'],
