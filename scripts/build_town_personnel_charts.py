#!/usr/bin/env python3
"""Three charts for who runs the town: where the posts are, how they move, who staffs up.
...
def chart_over_time(d):
    ...
    series = {k: [counts[y][k] for y in years] for k in
              ('elected board seat', 'appointed board seat', 'appointed officer')}
    W, H = 720, 320
    ...
    _lines(b, series, years, Y, cx,
           [ELECTED, APPOINTED, OFFICER],
           {'elected board seat': 'elected seats',
            'appointed board seat': 'appointed seats',
            'appointed officer': 'officers'})
...
CHARTS = [('town-personnel-where.svg', chart_where),
          ('town-personnel-over-time.svg', chart_over_time),
