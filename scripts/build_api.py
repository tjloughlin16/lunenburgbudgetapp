...
APP_FUNCTIONS = os.path.join(ROOT, 'fy28', 'functions', 'api')
...
def main():
    ...
    # A cost table for /api/query, baked in at build time.
    #
    # D1 bills on ROWS READ and offers no cap; the free plan simply stops at 5 million a
    # day. Measured: a COUNT over budget_figure reads 9,503 rows, and one join reads
    # 19,006 -- so 263 of those exhaust a day. One enthusiastic agent could do that in
    # minutes, and the endpoint would be dark until tomorrow.
    #
    # Nothing can know a query's true cost before running it. But the row count of every
    # table IS known here, and the tables a statement names can be read off it, so a
    # cheap upper estimate is available: the sum of the tables it touches, multiplied for
    # each join. That is an ESTIMATE and is labelled as one wherever it is used.
    sizes = {t: counts.get(t, 0) for t in counts}
    for v in views:
        sizes[v] = max((counts.get(t, 0) for t in counts), default=0)
    # ...and the UNIQUE columns, which is what stops that estimate refusing cheap joins.
    #
    # Multiplying is right for a join that can pair every row with every row. It is wrong
    # for an equi-join on a unique key, where each row matches AT MOST ONE: joining
    # `ledger_snapshot` to `account` on account_id cannot read more than the two tables
    # together, and estimating 983 x 981 = 964,323 refused a query that actually costs
    # about 1,964. That refusal is what TJ hit opening ledger_snapshot.
    #
    # So the estimate needs to know which columns are unique. Taken from the schema
    # itself -- unique indexes, plus declared primary keys -- rather than assumed from
    # names, because a column called `id` that is not unique would make the estimate
    # optimistic, and an optimistic cost guard is worse than none.
    uniq = {}
    for t in counts:
        cols = set()
        try:
            info = list(db.execute('PRAGMA table_info("%s")' % t))
        except Exception:
            continue
        pk = [r[1] for r in info if r[5]]
        if len(pk) == 1:
            cols.add(pk[0])
        for idx in db.execute('PRAGMA index_list("%s")' % t):
            if not idx[2]:                       # not UNIQUE
                continue
            icols = [r[2] for r in db.execute('PRAGMA index_info("%s")' % idx[1])]
            if len(icols) == 1 and icols[0]:
                cols.add(icols[0])
        if cols:
            uniq[t] = sorted(cols)
    with open(os.path.join(APP_FUNCTIONS, '_tablesizes.js'), 'w') as fh:
        fh.write('// Generated by scripts/build_api.py. Row count per table, for the cost\n'
                 '// estimate in query.js. A view is given the largest table\'s count,\n'
                 '// because what a view reads cannot be known from its name.\n'
                 'export const ROWS = ' + json.dumps(sizes, indent=1, sort_keys=True)
                 + '\n\n'
                 ...
                 'export const UNIQUE = ' + json.dumps(uniq, indent=1, sort_keys=True)
                 + '\n')
    print(f'  wrote functions/api/_tablesizes.js  ({len(sizes)} tables and views, '
          f'{len(uniq)} with a unique column)')
