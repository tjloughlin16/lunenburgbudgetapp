...
GROUPS = [
        ...
        ]},
    {
        ...
        'items': [
            ('state-census/tigerweb-cousub-massachusetts.json',
             'Town boundaries and centroids, every Massachusetts municipality', 1,
             'The Census Bureau’s county subdivisions for the whole state, as GeoJSON. '
             'It answers two questions no list can: which towns actually share a border '
             'with Lunenburg, and where each town is on a map. Every shape and pin on '
             '/analysis/towns-like-us comes out of this file.'),
            ('state-census/tigerweb-cousub-worcester-middlesex.json',
             'Town boundaries, Worcester and Middlesex counties', 1,
             'The first, narrower fetch of the same service — the one the six '
             'bordering towns were computed from by shared boundary vertices, which is how '
             'Ashby was found to have been missing from every earlier comparison.'),
        ...
        ]},
    {
        ...
        'items': [
            ...
            ('state-dls/OverrideUnderrideVotes.xlsx',
             'Override and Underride Votes, every municipality', 1,
             'Every Proposition 2½ question the state records — 4,743 of them '
             'across 305 municipalities — with the date, the tallies, the department '
             'and the result. NO export address is recorded: the report is not linked from '
             'the Gateway front page and we did not make the request, so the publisher’s '
             'own filename is what a resident asks DLS for.'),
            ('data/ingest-pending.csv',
             'Documents in flight: fetched, not yet in the bucket', 2,
             'The register `scripts/ingest.py` writes when it stages a document and clears '
             'when the bucket has read it back and matched its sha256. It is TRACKED on '
            ...
            ('data/archive-orphans.csv',
             ...
             'that leaves the manifest — usually a rename — leaves an object '
            ...
            ('data/dls-override-votes.csv',
             ...
             '`Win / Loss` is WIN exactly when the yes votes exceed the no votes, in all '
             '4,743 rows. scripts/extract_dls_overrides.py refuses to write if that ever '
             'stops holding.'),
    ...
    },
    {
        ...
        'items': [
            ...
            ('data/board-goals.csv',
             'What each board adopted as its goals, and when', 2,
             'One row per objective: the board, the fiscal year, the goal and objective '
             'text, the thread it belongs to, and the meeting that adopted it with its '
             'date and address.'),
            ('data/pec-history.csv',
             'The Public Employees Committee agreement, event by event', 2,
             'Dated events in the town\u2019s PEC agreement \u2014 the body that governs '
             'health insurance plan design and the contribution split under c.32B '
             '\u00a7\u00a721-23 \u2014 each with the verbatim quote it rests on and the '
             'document it came from.'),
            ('data/threads.csv',
             'The matters this project tracks across boards', 2,
             'One row per thread: the question a resident is following, which boards touch '
             'it, the match rules that find it in the record, when it was registered, and '
             'whether it has closed and on what vote.'),
            ('data/threads-declined.csv',
             'Thread candidates that were rejected, and why', 2,
             'The counterpart to threads.csv, kept because a rejected candidate is a '
             'decision rather than an absence: what was proposed, who declined it, the '
             'reason, and what would justify revisiting it.'),
            ('data/town-meeting-votes.csv',
             'Every Town Meeting article and how it was voted', 2,
             'FY2011 onward: the article, its subject, the result, the amount as printed, '
             'what the Finance Committee and Select Board recommended, and the verbatim '
             'quote with the page it was read from.'),
            ('analyses/spending-compared.md',
             'How Lunenburg compares, and what matching would cost', 3,
             'Lunenburg spends less for each pupil than almost every district in the '
             'state \u2014 310th of 318 in FY2025 \u2014 and this turns that rank into '
             'dollars: what reaching the median, or matching any named district, would '
             'cost. Also what the town itself is REQUIRED to raise for each pupil beside '
             'what the state sends, which is the figure the regionalisation argument '
             'actually turns on. Generated by scripts/build_spending_target.py.'),
            ('analyses/open-seats.md',
             'Open seats: the town boards a resident could join', 3,
             'A LIST rather than a report \u2014 the one page here somebody reads and then '
             'does something about. Seats the town printed empty, then every filled seat by '
             'the year its term runs out. Regenerated by the daily refresh, and published '
             'as `/data/open-seats.json` so an assistant can answer the question directly.'),
            ('analyses/towns-like-us.md',
             'How Lunenburg compares: neighbours, peers, look-alikes and destinations', 3,
             'Four comparison sets, because they answer four questions: the six towns that '
             'share a border (computed from Census boundary polygons, not remembered), the '
             'five this project uses as peers, the towns the math picks out of all 351, and '
             'the places Lunenburg children are actually schooled. The finding the two '
             'cohorts exist for: no town is among the ten closest to Lunenburg on both what '
             'it IS and what it DOES.'),
            ('analyses/charts/towns-like-us-map.svg',
             'Who Lunenburg is compared with, and where they are', 1,
            ...
            ('analyses/charts/towns-like-us-drivers.svg',
             ...
             'Average home value against the bill is much the strongest; certified new '
             'growth against the bill is effectively nothing.'),
            ('analyses/board-composition.md',
             'Board composition: how the boards are made up, and how they turn over', 3,
             'Which bodies are biggest, how fast each changes hands, and whether the '
             'establishment is growing. The finding the charts exist for: the seats are '
             'nearly flat across ten years while about a third of the PEOPLE in them change '
             'every year, and the town-wide figure hides a spread from 65% to nothing.'),
            ('analyses/charts/board-composition-where.svg',
             'Where the town\u2019s seats are, by body', 1,
            ...
            ('analyses/charts/town-budgets-trends.svg',
             ...
             'the shape of a $80,000 line is as legible as a $25M one. Eleven rise; only '
             'maturing debt falls.'),
            ('analyses/town-personnel.md',
             'Who runs the town: every elected seat, appointed seat and officer', 3,
             'The ELECTED OFFICIALS and APPOINTED OFFICIALS listing read as data, FY2022 '
             'to FY2025. Two thirds of the posts the town lists are unpaid seats on '
             'boards. It is NOT a headcount of town employees \u2014 a DPW labourer or a '
             'town hall clerk holds no appointed post \u2014 and it says so, because the '
             'wage list that would give a headcount stopped naming departments after '
             'FY2016.'),
            ('analyses/town-budgets.md',
             'Budgets across town: every department, and which ones move the total', 3,
             'What Town Meeting voted for each of the twelve departments in the omnibus '
             'budget, for the three years the detail reconciles, beside the voted total '
             'for every year from FY2012. It refuses the question it is most often asked '
             '\u2014 an omnibus budget cannot show a deficit, because a town may not vote '
             'one \u2014 and answers the useful version instead: ten of twelve departments '
             'grew faster than the levy cap, and the schools are the biggest line without '
             'being the fastest.'),
            ('analyses/town-budget-maturing-debt.md',
             'Maturing Debt & Interest: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-unclassified.md',
             'Employee benefits & reserves: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-general-government.md',
             'General Government: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-central-purchasing.md',
             'Central Purchasing: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-protection.md',
             'Protection: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-health-sanitation.md',
             'Health & Sanitation: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-public-works.md',
             'Public Works: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-facilities-grounds.md',
             'Facilities & Grounds: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-solid-waste.md',
             'Solid Waste & Recycling: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-assistance.md',
             'Assistance: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-schools.md',
             'Schools: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('analyses/town-budget-library.md',
             'Library: what the town votes for it', 3,
             'One of the twelve departments in the omnibus budget, with every '
             'budget line the town prints beneath it for FY2023, FY2024 and '
             'FY2025, the lines reconciled against the department total the page '
             'prints, and the lines that moved most between the first year and '
             'the last. Generated by scripts/build_town_budgets.py.'),
            ('data/department-staffing.csv',
             'What each town department says about its own staffing, verbatim', 2,
             'Departments describe their staffing in the PROSE of their annual-report '
             'sections \u2014 no heading names it in any year, which is why it went '
             'unread. The sentence is stored as printed because that is the source; counts '
             'are parsed into their own columns only where the form is unambiguous, and a '
             'statement that cannot be counted still gets a row with `parsed=no`. The forms '
             'do not agree with each other \u2014 a count and a range from the Fire '
             'Department, an establishment post by post from the DPW, a list of names from '
             'the Building Department \u2014 so `measure` says which kind each row is and '
             'nothing may be summed across kinds.'),
            ('data/department-rosters.csv',
             'The Police and Fire rosters, by name, as the town prints them', 2,
             'Both departments list every officer and firefighter by rank and assignment in '
             'every annual report. Read from the WORD geometry because both are set in two '
             'columns. The Fire Department also states its strength in a sentence, so the '
            ...
            ('data/town-personnel.csv',
             ...
             'annual report: every board and appointed post, its holders and their '
             'term-expiry years. Split the town\u2019s own way \u2014 elected board seat, '
             'appointed board seat, appointed officer \u2014 from the section header and '
             'from whether the post states a membership. `size_check` splits the rows '
             'three ways against the size each heading states, and nothing may be counted '
             'without splitting on it.'),
            ('data/table-corrections.csv',
             'Cells read off the page image, and cells checked against it', 2,
             'Two kinds of row. `read` says OCR got a cell wrong and gives what the page '
             'prints, with the page that was looked at. `attested` says OCR got it RIGHT '
             'and the DOCUMENT is what does not add up \u2014 FY2023 prints a General '
             'Government total 80 cents under its own lines \u2014 so nobody re-reads a '
             'page hunting a fault that was never there. A row matching nothing fails the '
             'extractor.'),
            ('analyses/stabilization-option.md',
             'Solution options: what the stabilization funds could do about the gap', 3,
             'The companion to the stabilization report, and deliberately separate from '
             'it: that page describes what the town holds, this one is only about what '
             'can be done with it. Three questions in order \u2014 can the town stop '
             'putting money in, can it spend what is already there, and then what. '
             'Spending the whole spendable balance closes two years and runs out partway '
             'through the third; stopping the deposits is recurring money, which is the '
             'right shape, and covers about a quarter of the first year\u2019s gap. '
             'Neither closes it. The gap comes from model/finance.py, the balances from '
             'the town\u2019s general ledger, the deposits from the articles that made '
             'them. See scripts/build_stabilization_option.py.'),
            ('analyses/stabilization-funds.md',
             'The stabilization funds, and who may spend them', 3,
             'What the town holds in reserve, which of it could lawfully go to an '
             'operating deficit \u2014 the general Stabilization Fund may, by a '
             'two-thirds Town Meeting vote, and the special purpose funds may not \u2014 '
             'and the four questions about movement the archive cannot yet answer because '
             'the balances are extracted but unreconciled. Generated by '
             'scripts/build_stabilization.py.'),
            ...
            ('analyses/charts/stabilization-flows.svg',
             'Money in and money out of the stabilization funds, by year', 1,
             'A diverging bar: deposits above the zero line, withdrawals below, one axis. '
             'It answers how OFTEN these funds are drawn on and for how much \u2014 eight '
             'withdrawals in thirteen years, none of them large beside the deposits. The '
             'balances are deliberately NOT on it: they are the same unit and thirty '
             'times the size, so showing them here would need a second y-axis.'),
            ...
            ('analyses/charts/town-personnel-where.svg',
             'Where the town\u2019s seats are, by body', 1,
             'A pie of every filled post in the listing, ten largest named and the long '
             'tail grouped. It found a defect the table had hidden: a body called `MILES` '
             'with 27 members, which was the town PROFILE page \u2014 `ROAD MILES- ABOUT '
             '100` read as a heading and the prose beneath it read as people.'),
            ('analyses/charts/town-personnel-over-time.svg',
             'Posts by kind, ten years \u2014 three nearly flat lines', 1,
            ...
            ('analyses/charts/town-budgets-pull.svg',
             ...
             'Diverging bars, one per department, in DOLLARS A YEAR above what the 2.5% levy '
             'cap would carry \u2014 the first version drew an index and called it points of '
             'total growth, which claimed an additivity it never had. It is the only chart on the page that '
             'puts employee benefits above the schools, which is the finding \u2014 a chart '
             'of SIZE would put the schools on top and say nothing.'),
            ('analyses/charts/town-budgets-total.svg',
             'What Town Meeting voted, every year from FY2012', 1,
             'One column per fiscal year, from the GRAND TOTAL each annual report prints '
             'at the foot of its omnibus. The last column is amber because FY2026 is a '
             'total and nothing else: no department table is printed for it anywhere.'),
    ...
    },
    {
        ...
        ('data/budget-cycles.csv',
         'When each budget season opens and closes: the day after one annual election to the next', 2,
         'Written by us from the election dates in the annual town reports and the 2026 printed '
         'results. scripts/budget_cycles.py reads it; a fiscal year’s season is FY(n): after the '
         'election of year n−2 to the election of year n−1.'),
        ('data/budget-episodes.csv',
         'The episodes of a budget season: the regular cycle and the special ones', 2,
         'Written by us. FY27’s season; the summer of 2026 (the Governor’s budget, the extra '
         'state aid, the September Special Town Meeting); FY28’s season. Each with its window, '
         'what triggered it and how it ended.'),
        ('data/budget-threads.csv',
         'The stories a budget season moves forward, one thread each', 2,
         'Written by us. The gap, the override, the cuts, the warrant articles, state aid; '
         'and a season-specific thread when TJ and the detector agree one has formed (FY27: '
         'the two override tiers). Each with its pattern and the question it answers.'),
        ('data/caption-corrections.csv',
         'Proper nouns the captions mishear, spelled as the town spells them', 2,
         'Written by us, each row with its basis in the town’s own documents. Names only; '
         'a figure as heard is never corrected.'),
        ('data/budget-seasons/fy27.csv',
         'The FY27 budget season as a status board: the gap’s path, the choices, every cut, the override, what took effect', 2,
         'Written by us, by hand, every row citing the event in our record (board/date@second) '
         'or the document it is read from. notes/process/BUDGET-SEASON-MODEL.md is the model.'),
        ('data/budget-seasons/fy26.csv',
         'The FY26 budget season as a status board: the $3.49M target gap, 39 positions cut, the petition override Town Meeting approved and the Select Board never put to voters', 2,
         'Written by us, by hand, from the district’s FY26 budget documents, the town’s minutes and our minutes of the recordings; every row cites its source.'),
        ('data/budget-seasons/fy27-summer-governors-budget.csv',
         'The summer of 2026 as a status board: $471,121 more state aid, the $350,000 then $418,056 ask, the 3 September Special Town Meeting, what came back', 2,
         'Written by us from our record of the May–September meetings and the analysis; every row cites its source. An episode, built the same way as a season.'),
        ('data/budget-seasons/fy28.csv',
         'The FY28 budget season, live: what is on the record so far, and the moments not yet come with when they usually do', 2,
         'Written by us. Confirmed rows only; warnings are read straight from the record; figures the record proposes wait in fy28.proposed.csv until a person copies them in.'),
        ('data/budget-seasons/fy28.proposed.csv',
         'Rows the record proposes for the FY28 board, awaiting confirmation', 2,
         'Generated daily by build_budget_season.py; never rendered. A row moves to fy28.csv when a person confirms it.'),
        ('data/budget-seasons/fy27-lines.csv',
         'Every budget line the FY27 scenarios differ on, school and town', 2,
         'Generated by extract_scenario_lines.py from the district’s line-item budget and the '
         'town’s Balanced/Tier 1/Tier 2 operating budgets, each column footed to the document’s '
         'printed total.'),
...
# A SUCCESSIVE EDITION OF AN ALREADY-CATALOGUED DOCUMENT NEEDS NO ENTRY OF ITS OWN.
#
# The DLS Gateway exports are named `<stem>-<date>-<first twelve of the sha256><ext>`,
# because they are FROZEN objects and a key can never be reused -- see `export_name` in
# fetch_dls_tax_bills.py. So every re-fetch is a new file, and DLS adds a fiscal year every
# cycle: cataloguing each one by hand means the catalogue grows a paragraph a quarter
# describing the same report, and the check that guards it fails until somebody does.
#
# This says instead: a file whose name is a catalogued stem plus a date and a hash is
# another edition of that document, and the catalogue's entry for the stem covers it. The
# CATALOGUE still names the edition the figures were read from -- `write_index` in each
# fetcher records the file it actually fetched -- and the manifest holds every one with its
# own sha256. What this skips is a second, hand-written description of the same report.
...
SKIP_FILES = {'supplemental.csv',
              ...
              # address, in the one file whose job is to keep it unpromoted. The mechanism
