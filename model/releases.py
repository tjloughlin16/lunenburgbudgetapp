...
RELEASES = [
    ...
    dict(
        ...
        short='budget to ledger, and where it stops',
        headline='A new analysis sets out what can be followed from what was budgeted to '
                 'what was spent. Two levels join. The third cannot, by anybody, and the '
                 'format a report arrives in decides which.',
        changes=[
            'The department as a whole reconciles between the two documents. By category — '
            '2710 Guidance, 2305 Teachers — 41 of 45 codes match, and only for FY2026 '
            'period 12. Below that nothing joins at all: the town’s system shortens account '
            'names to ten characters, so the middle school and high school guidance lines '
            'are the same string where the budget has a row for each.',

            'The format of a report decides what can be asked of it. A spreadsheet export '
            'carries the whole coded account and the printed PDF carries a label with the '
            'code removed — and that code is the only thing joining the district’s budget '
            'to the town’s ledger. One period can be compared because one report arrived as '
            'a spreadsheet.',

            'Whether state aid can be traced to what it paid for is published as a question '
            'rather than a finding. No revenue account carries a category code and state aid '
            'shares no organisation code with any expense account; the reading of that — '
            'that aid arrives unrestricted and nothing records which dollar paid which bill '
            '— is a claim about the town’s system made from outside it, and is labelled as '
            'inference.',

            'Underneath it, the database now carries the code that makes the category '
            'comparison possible. It was in the extracted data all along and was discarded '
            'every time the database was built. A check now fails the build if it goes '
            'missing again.',

            'And 8,598 budget figures — the largest table here, and most of what this site '
            'quotes — pointed at source documents by a name that matched none of them. '
            'Every figure now resolves to the document it came from.',
        ],
    ),
    dict(
        tag='v9.2.2',
        date='2026-09-03',
        title='The links for machines were at the bottom of the page, where nothing reads',
        short='machine-readable links, at the top',
        headline='v9.1 put every published address into the link graph. It put them in the '
                 'footer, and the footer of a 250KB page is not delivered to anything that '
                 'reads a page and stops. They are now the first thing under the header.',
        changes=[
            'An assistant asked to analyse the budget data read this site, was told by '
            'llms.txt that a data API exists, and could not fetch it: "the fetch tool is '
            'refusing every URL on the site except the two I already pulled." The links '
            'were live the whole time. They sat 95% of the way down a 250KB page, and the '
            'tools that read pages stop long before that.',

            'Measured rather than assumed: the front page is 255,823 bytes and the link to '
            'the data API was at byte 243,571. For comparison, a 121KB endpoint on this '
            'site comes back from the same kind of tool truncated in the middle of a '
            'record. The footer was never going to arrive.',

            'Every page now carries one line under the header naming the four addresses '
            'that matter — the index of everything, the guide, the API and the meeting '
            'archive. The footer keeps its copy, because people do read footers.',

            'The check that was supposed to catch this asked whether a link existed '
            'anywhere on the page, which is the assumption that failed. It now measures '
            'how far into the page each link sits and fails the build past a threshold. '
            'Run against the previous build it fails on all four, which is the only way to '
            'know a check works.',

            'Nothing a person reads has changed except one line of small type, and no '
            'figure has moved.',
        ],
    ),
    dict(
