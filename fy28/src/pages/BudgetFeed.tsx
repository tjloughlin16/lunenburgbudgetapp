...
type Statement = { board: string; board_slug: string; date: string; page: string; document?: boolean; thread?: string; kind: string; scope: string; fiscal_year: number | null; amount_as_heard: string | null; statement: string; who: string; status: string; t: number; video_url: string }
type Cut = { board: string; board_slug: string; date: string; page: string; document?: boolean; thread?: string; item: string; scope: string; fiscal_year: number | null; amount_as_heard: string | null; fte_as_heard: string | null; status: string; who: string; t: number; video_url: string; key: string }
type Warning = { board: string; board_slug: string; date: string; page: string; document?: boolean; thread?: string; prediction: string; about: string; condition: string | null; scope: string; fiscal_year: number | null; who: string; t: number; video_url: string }
...
function Line({ amount, text, who, board, board_slug, date, video_url, t, muted, quiet, chip, chipColor, after, landing, document, page }: { amount?: string | null; text: string; who?: string; board: string; board_slug: string; date: string; video_url?: string; t?: number | null; muted?: boolean; quiet?: boolean; chip?: string; chipColor?: string; after?: string; landing?: boolean; document?: boolean; page?: string }) {
  // A DOCUMENT'S LINE (write_document_budget_state.py): the source is the document, the
  // citation is its page, and "the board" is the publisher. TJ, 16 September 2026:
  // "anytime a new doc is 'found', we need to identify if it impacts the budget feed
  // and process it right then and there."
  const src = document
    ? <><a className="underline" href={page}>{board}</a>, {mmdd(date)}</>
    : <><a className="underline" href={`/boards/${board_slug}`}>{board}</a>, {mmdd(date)}</>
  const at = document ? (t ? `p. ${t}` : 'the document') : (ts(t) || 'video')
  return (
    <li className="pl-3 py-0.5" style={{ borderLeft: `${landing ? '4px' : '2px'} solid ${landing ? 'var(--status-good)' : muted || quiet ? 'var(--grid)' : (chipColor || 'var(--grid)')}`, color: muted ? 'var(--text-muted)' : quiet ? 'var(--text-secondary)' : undefined, background: landing ? 'color-mix(in srgb, var(--status-good) 8%, transparent)' : undefined }}>
      ...
      <span className="text-xs ml-1.5" style={{ color: 'var(--text-muted)' }}>{who ? `${who}, ` : ''}{chip ? null : src}{video_url ? <>{chip ? '' : ' · '}<a className="underline tnum" href={video_url}>{at}</a></> : null}</span>
...
function Tiers({ st, closed, decisions, outcome, finalText, finalNote, fy, meta }: { st: State; closed: boolean; decisions: Entry[]; outcome?: Payload['outcome'] | null; finalText?: string | null; finalNote?: string | null; fy: number; meta: Payload['threads'] }) {
  ...
  const story: Row[] = [
    ...all.map((x, i): Row => {
      ...
      return { date: x.date, t: x.t, thread: x.thread || 'other', vote: voted, nums: amounts(x.amount_as_heard || ''), render: landing => <Line key={'s' + i} chip={voted ? `${x.board} vote` : saidChip(x.status, x.who)} chipColor={voted ? 'var(--series-cost)' : 'var(--text-muted)'} quiet={!voted} muted={(superseded || x.status === 'withdrawn') && !landing} landing={landing} amount={x.amount_as_heard} text={`${x.statement} (${plain(x)})`} board={x.board} board_slug={x.board_slug} date={x.date} video_url={x.video_url} t={x.t} document={x.document} page={x.page} after={superseded && !landing ? `overwritten ${mmdd(later.date)}${later.amount_as_heard ? ` → ${later.amount_as_heard}` : ''}` : undefined} /> }
    }),
    ...st.cuts.map((c, i): Row => {
      const voted = c.status === 'voted', gone = c.status === 'restored' || c.status === 'withdrawn'
      return { date: c.date, t: c.t, thread: c.thread || 'other', vote: voted, nums: voted ? amounts(c.amount_as_heard || '') : [], render: landing => <Line key={'c' + i} chip={voted ? `${c.board} vote` : gone ? `${c.status} · ${c.who}` : (c as Cut & { aftermath?: boolean }).aftermath ? `after the ballot · ${c.who}` : `proposed cut · ${c.who}`} chipColor={voted ? 'var(--series-cost)' : 'var(--text-muted)'} quiet={!voted} muted={gone} landing={landing} amount={c.amount_as_heard || c.fte_as_heard && `${c.fte_as_heard} FTE` || null} text={`${gone ? c.status + ': ' : 'cut: '}${c.item}`} board={c.board} board_slug={c.board_slug} date={c.date} video_url={c.video_url} t={c.t} document={c.document} page={c.page} /> }
    }),
    ...(st.warnings || []).map((w, i): Row => ({ date: w.date, t: w.t, thread: w.thread || 'other', vote: false, nums: [], render: () => <Line key={'w' + i} chip={`warned · ${w.who}`} chipColor="var(--status-warning)" quiet text={`${w.prediction}${w.condition ? ` — ${w.condition}` : ''} (${SCOPE[w.scope] || w.scope}: ${w.about})`} board={w.board} board_slug={w.board_slug} date={w.date} video_url={w.video_url} t={w.t} document={w.document} page={w.page} /> })),
