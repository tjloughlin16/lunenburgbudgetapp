...
export const RATE_LINES: RateLine[] = ([
  ...
  ['sped', 'Special education, in district',
   ...
   'About a fifth of the budget, and the second-largest driver of the gap after insurance. It was invisible until now because the state’s account codes file it under salaries, where it inherited the teachers’ contract rate. There is no special education contract: professional staff are on the teachers’ agreement and paras on the paraprofessionals’, and the default here is those contracts weighted by how much of the line each covers. The line itself rose faster over the last two budgets — but that was one 39% increase in paras, which is a step rather than a rate, and it is already inside the amount this model starts from. Bend the Curve sets out the whole argument and the range around it.'],
