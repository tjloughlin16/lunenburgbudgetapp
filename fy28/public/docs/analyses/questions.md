...
```sql
SELECT fy, v1 AS appropriated, column_meaning, status FROM report_appropriations WHERE label = 'Monty Tech Assessment' AND table_family = 'accountant-schedule' ORDER BY fy DESC LIMIT 20
```

Returns `fy`, `appropriated`, `column_meaning`, `status` — for example: fy=2023, appropriated=1054376.00, column_meaning=not established -- this page states no identity that fixes which printed column is which, status=check failed
...
Returns `source_type`, `basis`, `documents` — for example: source_type=primary, basis=None, documents=1096
...
Returns `source_type`, `documents` — for example: source_type=primary, documents=1096
