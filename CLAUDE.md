...
## 7d. Read the page three times as the resident it is for. Each pass finds a different class of fault

The FY27 budget season page was built correct and read three times as a parent whose
child is in fifth-grade band and plays middle school sports. Every pass changed the page,
and the faults were different in kind each time -- which is the reason it is three passes
and not one. The rules below are what those passes generalise to. They apply to every
page that tells a resident where something stands, and they apply LIVE: the refresh will
propose rows into the same season file, and a proposed row that breaks one of these is
wrong before anyone confirms it.

**Pass 1 -- is the first number the one people said?** The page led with *$761,001*, a
spreadsheet difference (level service minus balanced) that nobody uttered all spring. The
town said $2.4 million, then $1.98 million. A derived figure is context; the stated figure
is the headline, with who said it and when. *A reader who hears a number they never heard
assumes the page is wrong, not that it is precise.*

**Pass 1 -- do our notes leak?** "The mistake TJ remembers", "corrected 14 Sep against
the district's page" -- audit notes for us, rendered to the town. A note that is about our
process is prefixed `internal:` in the data and never rendered. The audit trail is for the
CSV; the page gets the finding.

**Pass 1 -- does one fact end two ways?** FINAL said "middle school sports gone" three
lines above "restored after the ballot". Both were true in sequence and the first was
worded as an end state. **A fact that changes carries its date range** ("stood from 16
May", "until the September restoration below"), and every block that states it agrees.

**Pass 2 -- does each choice say what it buys, in the words of the cuts?** The tiers
showed dollars and FTE. A parent's question is "what would a yes have bought my kid", and
the answer was on the district's own page: *Core brings back a kindergarten para, an
interventionist at Turkey Hill, the bridge program...* A proposal is described by the
named things it restores or takes, never by its total alone.

**Pass 2 -- does every track end where it ended?** The override track stopped at the
Select Board's recommendation on 7 April; the reader had to find "failed" in another
block. A sequence ends with its outcome, on the same list.

**Pass 2 -- is the same thing said twice in two vocabularies?** The agenda-window
calendar sat under the board saying "Budget vote: Jun 3 '25 -> Mar 23 '26"; the
department name appeared under the department heading; the 864-line log sat under the
finished story. One structure per fact. When a better structure exists, the older one is
folded or removed, not stacked beneath.

**Pass 3 -- is every item findable by the word a resident would search?** Girls'
lacrosse, boys' golf and the ski team were on the district's reduction list and were only
IMPLIED here inside "middle school sports -- all". The test is mechanical: take the
publisher's own list and check every name on it has a row. A family looks for its own
word, and an item that is only inside another item is not there.

**Pass 3 -- is a status about the world or about our data?** "Named 26 Feb; not on the
3/23 balanced list" describes our tables. "Proposed 26 Feb; gone from the 23 Mar list --
the record gives no reason" describes what happened AND what we could not find, which is
the honest version of rule 7 at the level of one line.

**Pass 3 -- is a fee a cut?** "Coaches -- stipends offset by a $75 fee rise" was filed
as a cut. A parent needs "the athletic fee goes up $75". The thing that changes for the
reader leads; the accounting follows.

**The general form.** Pass 1 finds the page's own errors (wrong headline, leaked notes,
contradictions). Pass 2 finds what the page withholds (what a choice buys, how a track
ends, the structure stacked beneath). Pass 3 finds what a specific reader cannot find
(their word, the honest "we don't know", the thing mislabelled). They do not collapse
into one read because each needs the previous one fixed to become visible. Record each
pass in `notes/process/PERSONAS.md` under Reviews run, with what it changed.
